...
const checkOverlappingSessions = async (
  studentId,
  instructorId,
  startTime,
  endTime
  ...
  const studentOverlap = await Registration.findOne({
    studentId,
    status: 'scheduled',
  ...
  if (studentOverlap) {
    errors.push('student has an overlapping class scheduled');
  }

  // Check for instructor overlap
  const instructorOverlap = await Registration.findOne({
    instructorId,
    status: 'scheduled',
    startTime: { $lt: endTime },
    endTime: { $gt: startTime },
  ...
  if (instructorOverlap) {
    errors.push('instructor has an overlapping class scheduled');
  }
...
const checkDailyLimits = async (
  ...
  const maxStudentClasses =
    parseInt(process.env.MAX_STUDENT_CLASSES_PER_DAY) || 3;
  ...
  const maxInstructorClasses =
    parseInt(process.env.MAX_INSTRUCTOR_CLASSES_PER_DAY) || 5;
  if (instructorDailyCount >= maxInstructorClasses) {
    throw new Error(
      `Instructor daily limit exceeded (max: ${maxInstructorClasses})`
    );
  }
};
...
const checkClassTypeCapacity = async (classType, startTime) => {
  ...
  if (classTypeDailyCount >= maxClassesPerType) {
    throw new Error(
      `Class type daily capacity exceeded (max: ${maxClassesPerType})`
    );
  }
};
